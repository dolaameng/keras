...
Notes
- What:
    + Net2Net is a group of methods to transfer knowledge from a teacher neural net to a student net, so that the student net can be trained faster than from scratch.
    + The paper discussed two specific methods of Net2Net, i.e. Net2WiderNet and Net2DeeperNet.
    + Net2WiderNet replaces a model with an equivalent wider model that has more units in each hidden layer.
    + Net2DeeperNet replaces a model with an equivalent deeper model.
    + Both are based on the idea of "function-preserving transformations of neural nets".
- Why:
    + Enable fast exploration of multiple neural nets in experimentation and design process, by creating a series of wider and deeper models with transferable knowledge.
    + Enable "lifelong learning system" by gradually adjusting model complexity to data availability, and reusing transferable knowledge.

Experiments
- Teacher model: a basic CNN model trained on MNIST for 3 epochs.
- Net2WiderNet exepriment:
    - Student model has a wider Conv2D layer and a wider FC layer, compared to teacher model.
    - Comparison of two weight-transfer methods: "random-padding" vs "net2wider".
    - With both methods, student model should immediately perform as well as teacher model, but 'net2wider' is slightly better.
- Net2DeeperNet experiment:
    - Student model has an extra Conv2D layer and an extra FC layer, compared to teacher model.
    - Comparison of two weight-transfer methods: "random-init" vs "net2deeper".
    - With 'net2deeper', student model should immediately perform as well as teacher model.
- Hyper-parameters:
    - Simple SGD is used intentionally to show the continuous learning of student model, given that MNIST is a simple dataset.
    - Learning rate adjustment: it's suggested to reduce learning rate to 1/10 for student model, but not implemented here.
    - Addition of noise in "net2wider" is used to break weight symmetry and thus enable full capacity of student models. It is optional when a Dropout layer is used.

Results
- Tested with "Theano" backend and "th" image_dim_ordering.
- Running on GPU GeForce GTX 980
- Performance Comparisons - loss values during first 3 epochs: ????
...
def wider2net_conv2d(teacher_w1, teacher_b1, teacher_w2, new_width, init):
    ...
    # Auguments
        teacher_w1: `weight` of conv2d layer to become wider, of shape (nb_filter1, nb_channel1, kh1, kw1)
        teacher_b1: `bias` of conv2d layer to become wider, of shape (nb_filter1, )
        teacher_w2: `weight` of next connected conv2d layer, of shape (nb_filter2, nb_channel2, kh2, kw2)
    ...
    """
    assert teacher_w1.shape[0] == teacher_w2.shape[1], "two successive layers from teacher model should have compatible shapes"
    assert teacher_w1.shape[0] == teacher_b1.shape[0], "weight and bias from same layer should have compatible shapes"
    assert new_width > teacher_w1.shape[0], "new width (nb_filter) should be bigger than the existing one"
...
def wider2net_fc(teacher_w1, teacher_b1, teacher_w2, new_width, init):
    ...
    # Auguments
        ...
        init: initialization algorithm for new weights, either 'random-pad' or 'net2wider'
    """
    assert teacher_w1.shape[1] == teacher_w2.shape[0], "two successive layers from teacher model should have compatible shapes"
    assert teacher_w1.shape[1] == teacher_b1.shape[0], "weight and bias from same layer should have compatible shapes"
    assert new_width > teacher_w1.shape[1], "new width (nout) should be bigger than the existing one"
...
def deeper2net_conv2d(teacher_w):
    ...
    # Auguments
        teacher_w: `weight` of previous conv2d layer, of shape (nb_filter, nb_channel, kh, kw)
    """
